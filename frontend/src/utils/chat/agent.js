...
/** 助手改 vault 后通知侧栏文件树刷新 */
export const VAULT_TREE_CHANGED_EVENT = "bagu_vault_tree_changed";
...
export default function handleSocketResponse(socket, event, setChatHistory) {
  ...
  // Agent filesystem mutations → refresh left vault tree
  if (data.type === "vaultTreeChanged") {
    try {
      window.dispatchEvent(
        new CustomEvent(VAULT_TREE_CHANGED_EVENT, {
          detail: data.content || {},
        })
      );
    } catch (_) {
