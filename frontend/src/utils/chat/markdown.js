...
// Theme-aware emphasis (do not hardcode white — breaks light theme)
markdown.renderer.rules.strong_open = () => '<strong class="md-strong">';
markdown.renderer.rules.strong_close = () => "</strong>";
markdown.renderer.rules.em_open = () => '<em class="md-em">';
markdown.renderer.rules.em_close = () => "</em>";
markdown.renderer.rules.link_open = (tokens, idx) => {
  const token = tokens[idx];
  const href = token.attrs.find((attr) => attr[0] === "href");
  return `<a href="${HTMLEncode(href[1])}" target="_blank" rel="noopener noreferrer" class="md-link">`;
};
