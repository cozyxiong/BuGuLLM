...
/** 工作区创建/更新后广播，侧栏等监听以同步名称等展示字段 */
export const WORKSPACE_UPDATED_EVENT = "bagu_workspace_updated";

function emitWorkspaceUpdated(workspace, { slug } = {}) {
  if (typeof window === "undefined" || !workspace) return;
  try {
    window.dispatchEvent(
      new CustomEvent(WORKSPACE_UPDATED_EVENT, {
        detail: {
          workspace,
...
const Workspace = {
  ...
  new: async function (data = {}) {
    ...
    if (workspace) emitWorkspaceUpdated(workspace);
  ...
  update: async function (slug, data = {}) {
    ...
    if (workspace) emitWorkspaceUpdated(workspace, { slug });
    return { workspace, message };
  },
    ...
    await fetchEventSource(`${API_BASE}/workspace/${slug}/stream-chat`, {
      ...
      async onopen(response) {
        if (response.ok) {
          ...
          const msg404 =
            response.status === 404
              ? "当前工作区或对话不存在，请刷新页面或新建对话后再试。"
              : `发送失败（HTTP ${response.status}）。请检查网络或后端服务是否正常。`;
          handleChat({
            id: v4(),
            type: "abort",
            textResponse: null,
  ...
  storeWorkspaceOrder: function (workspaceIds = []) {
    try {
      localStorage.setItem(
        ...
        JSON.stringify(workspaceIds.map((id) => String(id)))
      );
      return true;
    } catch (error) {
  ...
  orderWorkspaces: function (workspaces = []) {
    ...
    orderedWorkspaces.sort((a, b) => {
      const ai = workspaceOrderPreference.indexOf(String(a.id));
      const bi = workspaceOrderPreference.indexOf(String(b.id));
      // 新工作区（不在偏好里）排到末尾，避免 indexOf(-1) 乱序
