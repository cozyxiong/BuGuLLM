  ...
  return (
    <>
      <p className="text-sm font-base text-white text-opacity-60 mb-4">
        PiperTTS 的模型都在浏览器本地运行，低配设备上可能会比较吃资源。
      </p>
...
function voicesByLanguage(voices = []) {
  const voicesByLanguage = voices.reduce((acc, voice) => {
    const langName = voice?.language?.name_english ?? "未分类";
...
function voiceDisplayName(voice) {
  ...
  return `${is_stored ? "✔ " : ""}${titleCase(name)}-${quality === "low" ? "低质量" : "高清"} (${humanFileSize(fileSize)})`;
}
    ...
    >
      {speaking ? (
        <>
          ...
          <p className="text-sm flex-shrink-0">停止试听</p>
        </>
      ) : (
        <>
          {loading ? (
            <>
              ...
              <p className="text-sm flex-shrink-0">正在加载</p>
            </>
          ) : (
            <>
              <PlayCircle size={20} className="flex-shrink-0 text-white" />
              <p className="text-white text-sm flex-shrink-0">试听</p>
            </>
          )}
        </>
      )}
