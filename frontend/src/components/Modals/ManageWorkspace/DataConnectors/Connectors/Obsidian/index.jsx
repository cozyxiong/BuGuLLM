...
export default function ObsidianOptions({ onImport, workspaceSlug: workspaceSlugProp }) {
  const { t } = useTranslation();
  const { slug } = useParams();
  const workspaceSlug = workspaceSlugProp || slug;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data, library, error } = await System.dataConnectors.obsidian.collect({
        files: fileContents,
        workspaceSlug,
      });
      ...
      const libraryMsg = library?.message ? ` ${library.message}` : "";

      if (successCount === totalCount) {
        showToast(
          `Successfully imported ${successCount} files from your vault!${libraryMsg}`,
          "success",
          { clear: true }
        );
      } else {
        showToast(
          `Imported ${successCount} files, ${failCount} failed${libraryMsg}`,
          "warning",
          { clear: true }
        );
      }
