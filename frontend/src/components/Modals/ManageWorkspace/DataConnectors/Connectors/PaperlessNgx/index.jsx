...
export default function PaperlessNgxOptions({ onImport, workspaceSlug: workspaceSlugProp }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data, library, error } = await System.dataConnectors.paperlessNgx.collect({
      ...
      const libraryMsg = library?.message
        ? ` ${library.message}`
        : ` Output folder is ${data.destination}.`;
      showToast(
        `Successfully imported ${data.files} documents from Paperless-ngx.${libraryMsg}`,
        "success",
        { clear: true }
      );
      e.target.reset();
