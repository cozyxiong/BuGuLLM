...
export default function YoutubeOptions({ onImport, workspaceSlug: workspaceSlugProp }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data, library, error } = await System.dataConnectors.youtube.transcribe({
        url: form.get("url"),
        workspaceSlug,
      });
      ...
      const libraryMsg = library?.message
        ? ` ${library.message}`
        : library?.count
          ? ` 已写入知识库 ${library.folder}（${library.count} 个文件）`
          : "";

      showToast(
        `${data.title} by ${data.author} transcription completed.${libraryMsg || ` Output folder is ${data.destination}.`}`,
        "success",
        { clear: true }
      );
      e.target.reset();
