...
export default function useSkillSections({
  ...
  return useMemo(() => {
    ...
    // Agent Skills (default + configurable); 语义检索(rag-memory) 固定排第一
    const skillItems = [];
    const mergedSkills = { ...defaultSkills, ...configurableSkills };
    const skillKeys = Object.keys(mergedSkills).sort((a, b) => {
      if (a === "rag-memory") return -1;
      if (b === "rag-memory") return 1;
      return 0;
    });
    for (const key of skillKeys) {
      const { title } = mergedSkills[key];
      skillItems.push(
        buildSkillItem({
          key,
          title,
