...
export default function LLMSelectorSidePanel({
  ...
  return (
    <div
      className={
        compact
          ? "w-full flex flex-col gap-2 p-2.5 border-b border-zinc-700/80 light:border-slate-200"
          : "w-[40%] h-full flex flex-col gap-4 p-2 border-r border-zinc-700 light:border-slate-300"
      }
    >
      <div className={`relative shrink-0 ${compact ? "" : "mx-2"}`}>
        <MagnifyingGlass
          size={14}
      ...
      >
        {availableProviders.map((llm) => (
          <button
            ...
            className={`border-none cursor-pointer flex gap-2 items-center px-2.5 py-1.5 rounded-md transition-colors min-w-0 ${
          ...
          >
            ...
            />
            <span className="text-xs text-white light:text-slate-900 truncate whitespace-nowrap">
              {llm.name}
            </span>
          </button>
        ))}
