...
export default forwardRef(function (
  ...
  const renderStatusResponse = useCallback(
    (item, index) => {
      // Collapse tool chain once any real reply (or later group) follows it
...
const SESSION_COMPLETE_RE = /agent session complete/i;
...
const getLastMessageInfo = (history) => {
  ...
  // Websocket close appends a terminal status — never keep the activity strip "running"
  if (type === "statusResponse" && SESSION_COMPLETE_RE.test(content)) {
    return { isAnimating: false, isStatusResponse: true, isSessionComplete: true };
  }

  // Only treat as active when truly in-flight or waiting on the user.
  // Do NOT mark every statusResponse as animating (that left "处理中" stuck forever).
  const waitingOnUser =
    type === "toolApprovalRequest" || type === "clarifyingQuestion";
  const streamingTool = type === "toolCallInvocation";
  ...
  return {
    isAnimating:
      ...
      waitingOnUser ||
      streamingTool,
    isStatusResponse: type === "statusResponse",
...
function buildMessages({
  ...
  return history.reduce((acc, props, index) => {
    ...
    // Group status + streaming tool invocations into one Grok-style activity strip
    if (
      (props?.type === "statusResponse" ||
        props?.type === "toolCallInvocation") &&
      !!props.content
    ) {
    ...
    if (props.type === "rechartVisualize" && !!props.content) {
      ...
      acc.push(
        <PromptReply
          ...
          relatedTopics={props.relatedTopics}
