...
export default function ChatContainer({
      ...
      try {
        ...
        socket.addEventListener("close", (_event) => {
          ...
          if (pendingResetRef.current) {
            ...
            // Mark prior agent status/tool lines as finished; do not append a
            // visible "Agent session complete." row (it left the strip stuck on 处理中).
            setChatHistory((prev) =>
              prev.map((msg) =>
                msg?.type === "statusResponse" ||
                msg?.type === "toolCallInvocation"
                  ? { ...msg, animate: false, pending: false, closed: true }
                  : msg
              )
            );
          }
  ...
  const shellStyle = {
    height: fillPane || isMobile ? "100%" : "calc(100% - 32px)",
  };
  const shellClass = fillPane
    ? "relative flex w-full h-full z-[2]"
    : "relative flex md:ml-[2px] md:mr-[16px] md:my-[16px] w-full h-full z-[2]";
  const paneClass = fillPane
    ? "flex-1 min-w-0 relative bg-theme-bg-secondary w-full h-full overflow-hidden border-none"
    : "flex-1 min-w-0 relative md:rounded-[16px] bg-zinc-900 light:bg-white w-full h-full overflow-hidden border-none light:border-solid light:border light:border-theme-modal-border";
