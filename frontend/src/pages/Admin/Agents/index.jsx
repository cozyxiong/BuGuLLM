...
export default function AdminAgents() {
  ...
  useEffect(() => {
    async function fetchSettings() {
      try {
        ...
        const prefs = _preferences?.settings ?? {};
        const { flows = [] } = flowsRes || {};
        setSettings({ ...(_settings || {}), preferences: prefs });
        setAgentSkills(prefs.default_agent_skills ?? []);
        setDisabledAgentSkills(prefs.disabled_agent_skills ?? []);
        setImportedSkills(prefs.imported_agent_skills ?? []);
  ...
  if (loading) {
    return (
      <SkillLayout hasChanges={false} handleCancel={() => {}} handleSubmit={() => {}}>
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Configurable first so 语义检索(rag-memory) leads the list */}
            <SkillList
