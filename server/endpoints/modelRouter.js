...
function modelRouterEndpoints(app) {
  ...
  app.get(
    "/model-routers/:id/workspaces",
    [validatedRequest, flexUserRoleValid([ROLES.admin])],
    async (request, response) => {
      try {
        const { id } = request.params;
        const workspaces = await ModelRouter.workspaces(Number(id));
    ...
    async (request, response) => {
      try {
        ...
        if (!Array.isArray(slugs))
          return response
            .status(400)
            .json({ success: false, error: "slugs must be an array." });
        const workspaces = await ModelRouter.setWorkspaces(Number(id), slugs);
        return response.status(200).json({ success: true, workspaces });
      } catch (e) {
        console.error(e);
        response.sendStatus(500);
      }
