// 开发：先加载 .env.development 模板，再用 .env 覆盖（UI 保存的模型配置会 dump 到 .env，重启不丢）
// 生产：只加载 .env
...
const { libraryEndpoints } = require("./endpoints/libraries");
const { learningEndpoints } = require("./endpoints/learning");
const { generateEndpoints } = require("./endpoints/generate");
const { tagsEndpoints } = require("./endpoints/tags");
const { feishuEndpoints } = require("./endpoints/feishu");
const { videoEndpoints } = require("./endpoints/video");
...
learningEndpoints(apiRouter);
generateEndpoints(apiRouter);
tagsEndpoints(apiRouter);
feishuEndpoints(apiRouter);
videoEndpoints(apiRouter);
