...
class GenericOpenAiLLM {
  ...
  /** 学习生成用型号输出上限，不用聊天里的 Max Tokens */
  maxOutputLimit() {
    try {
      const { MODEL_MAP } = require("../modelMap");
      const mapped = MODEL_MAP.getOutput(null, this.model);
      if (mapped) return mapped;
    } catch {
      /* ignore */
    }
    const name = String(this.model || "").toLowerCase();
    if (/deepseek/i.test(name) && /v4/.test(name)) return 384_000;
    return null;
  }
  ...
  async getChatCompletion(
    ...
    const tokenCap =
      Number(maxTokens) > 0 ? Number(maxTokens) : this.maxTokens;
