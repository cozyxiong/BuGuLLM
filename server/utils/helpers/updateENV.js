...
async function updateENV(newENVs = {}, force = false, userId = null) {
  ...
  // 开发/生产都落盘：否则开发模式重启后 LLM/嵌入等 UI 配置会丢失（只留在内存 process.env）
  dumpENV();
  if (shouldRefreshModelMap(newValues)) {
    try {
      const { MODEL_MAP } = require("../AiProviders/modelMap");
      await MODEL_MAP.refresh();
    } catch (e) {
      console.error("Failed to refresh model context windows:", e.message);
    }
  }
  return { newValues, error: error?.length > 0 ? error : false };
}

function shouldRefreshModelMap(newValues = {}) {
  return Object.keys(newValues).some((key) => {
    if (key === "LLMProvider") return true;
    if (!key.includes("ModelPref")) return false;
    if (
...
function dumpENV() {
  ...
  const protectedKeys = [
    ...
    // Auth / app secrets commonly set outside KEY_MAPPING
    "JWT_SECRET",
    "AUTH_TOKEN",

    // BaGuLLM 扩展配置（避免 dump 时丢掉）
    "FEISHU_APP_ID",
    "FEISHU_APP_SECRET",
    "FEISHU_REDIRECT_URI",
    "BAGU_VAULT_DIR",
  ...
  envResult += `# Persisted UI/runtime settings. Loaded after .env.development in development mode.\n`;
  envResult += Object.entries(frozenEnvs)
    .map(([key, value]) => `${key}='${sanitizeValue(value)}'`)
    .join("\n");

  // 始终写入 server/.env（开发下 index.js 会以 override 方式二次加载）
