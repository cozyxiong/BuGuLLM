...
/**
 * 同步嵌入任务（非 native worker）或 embed-all 准备阶段计数。
 * @type {Map<string, number>}
 */
const syncEmbedRefCount = new Map();

function beginSyncEmbed(slug) {
  if (!slug) return;
  syncEmbedRefCount.set(slug, (syncEmbedRefCount.get(slug) || 0) + 1);
}

function endSyncEmbed(slug) {
  if (!slug) return;
  const n = (syncEmbedRefCount.get(slug) || 0) - 1;
  if (n <= 0) syncEmbedRefCount.delete(slug);
  else syncEmbedRefCount.set(slug, n);
}

/** 工作区是否仍有嵌入任务（worker 或同步请求）在进行 */
function isEmbeddingActive(slug) {
  if (!slug) return false;
  return (
    runningWorkers.has(slug) || (syncEmbedRefCount.get(slug) || 0) > 0
  );
}
