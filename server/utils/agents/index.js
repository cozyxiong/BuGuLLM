...
class AgentHandler {
  ...
  /**
   * 助手模式(assistant/automatic)下始终走 Agent；问答模式不走。
  ...
  static async isAgentInvocation({
    ...
    const mode = chatMode || workspace?.chatMode;
    if (mode === "assistant" || mode === "automatic") return true;
    // 问答模式不再支持 @agent 旁路
    return false;
  }
  ...
  async init() {
    ...
    // 助手文件工具：限定在工作区 vault 目录内（read/write/search/bash）
    try {
      const path = require("path");
      const fs = require("fs");
      const filesystem = require("./aibitat/plugins/filesystem/lib");
      const { VAULT_ROOT } = require("../libraryVault");
      const slug = this.invocation?.workspace?.slug;
      const dirs = [];
      if (slug) {
        const wsVault = path.join(VAULT_ROOT, slug);
        if (!fs.existsSync(wsVault)) fs.mkdirSync(wsVault, { recursive: true });
        dirs.push(wsVault);
      }
      if (dirs.length === 0) dirs.push(VAULT_ROOT);
      await filesystem.reinitialize(dirs);
      this.log(`Filesystem sandbox: ${dirs.join(", ")}`);
    } catch (e) {
      this.log(`Filesystem sandbox setup skipped: ${e.message}`);
    }
