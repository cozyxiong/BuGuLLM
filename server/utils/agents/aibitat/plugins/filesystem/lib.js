...
class FilesystemManager {
  ...
  /**
   * BaGuLLM 本地单用户：始终可用（不依赖 Docker / Claude Code）。
   * @returns {boolean}
   */
  isToolAvailable() {
    return true;
  ...
  /**
   * 默认操作根目录：知识库 vault（storage/vault）。
   * @returns {string}
   */
  #getDefaultFilesystemRoot() {
    try {
      const { VAULT_ROOT } = require("../../../../libraryVault");
      return VAULT_ROOT;
    } catch {
      const storageRoot =
        process.env.STORAGE_DIR ||
        path.resolve(__dirname, "../../../../../storage");
      return path.join(storageRoot, "vault");
    }
  }

  /**
   * 重新限定可访问目录（助手会话按工作区 vault 收紧）。
   * @param {string[]} directories
   * @returns {Promise<string[]>}
   */
  async reinitialize(directories = null) {
    this.#isInitialized = false;
    return await this.#initializeFilesystem(directories);
  }
  ...
  /**
   * Notify the frontend sidebar to refresh the vault file tree after mutations.
   * Model-agnostic: works for any agent provider via the agent websocket.
   * @param {object} aibitat - aibitat instance (this.super in plugins)
   * @param {object} [detail]
   */
  notifyVaultTreeChanged(aibitat, detail = {}) {
    try {
      aibitat?.socket?.send?.("vaultTreeChanged", {
        at: Date.now(),
        ...detail,
      });
    } catch (_) {
  ...
  async validatePath(requestedPath) {
    ...
    // Model-agnostic path hygiene: trim quotes, normalize slashes, map abs paths under vault
    let cleaned = String(requestedPath ?? "")
      .trim()
      .replace(/^["']|["']$/g, "");
    if (!cleaned || cleaned === ".") cleaned = ".";
    cleaned = cleaned.replace(/\\/g, "/");

    // If absolute path sits under an allowed dir, keep resolve; otherwise try as relative
    let candidate = cleaned;
    if (path.isAbsolute(cleaned) || /^[a-zA-Z]:\//.test(cleaned)) {
      const absWin = path.resolve(cleaned);
      const underAllowed = this.#allowedDirectories.some((dir) => {
        const nd = this.#normalizePath(path.resolve(dir));
        const na = this.#normalizePath(absWin);
        return na === nd || na.startsWith(nd + path.sep);
      });
      if (!underAllowed) {
        // Strip drive/root prefixes that models invent; treat remainder as vault-relative
        candidate = cleaned
          .replace(/^[a-zA-Z]:\//, "")
          .replace(/^\/+/, "")
          .replace(/^(?:.*?\/)?(?:vault\/[^/]+\/)/i, "");
    ...
    const expandedPath = this.#expandHome(candidate);
  ...
  /**
   * 在允许目录内执行 shell 命令（助手 bash，不依赖 Claude Code）。
   * @param {string} command
   * @param {{cwd?: string, timeoutMs?: number}} options
   * @returns {Promise<{stdout: string, stderr: string, exitCode: number, cwd: string}>}
   */
  async runCommand(command, { cwd = null, timeoutMs = 60_000 } = {}) {
    await this.ensureInitialized();
    const { spawn } = require("child_process");

    const raw = String(command || "").trim();
    if (!raw) throw new Error("command is required");
    if (raw.length > 4000) throw new Error("command is too long");

    // 粗粒度危险命令拦截（沙箱目录仍是主防线）
    const blocked =
      /\b(format\s+[a-z]:|mkfs\.|shutdown|reboot|rm\s+(-[a-zA-Z]*f[a-zA-Z]*\s+)?\/\s*$|del\s+\/[sf]\s+[a-z]:\\)/i;
    if (blocked.test(raw)) {
      throw new Error("Command blocked by safety policy.");
    }

    let workDir = this.#allowedDirectories[0];
    if (cwd) {
      workDir = await this.validatePath(cwd);
      const stats = await fs.stat(workDir);
      if (!stats.isDirectory()) {
        throw new Error("cwd must be a directory within the allowed vault.");
      }
    }

    const isWin = process.platform === "win32";
    const shell = isWin ? process.env.ComSpec || "cmd.exe" : "/bin/bash";
    const args = isWin ? ["/d", "/s", "/c", raw] : ["-lc", raw];
    ...
    return await new Promise((resolve, reject) => {
      const child = spawn(shell, args, {
        ...
        env: {
          ...process.env,
          // 降低把密钥扫进日志的风险：不额外注入；子进程继承现有 env
        },
        windowsHide: true,
      });
