    ...
    return {
      ...
      setup(aibitat) {
        aibitat.function({
          ...
          name: this.name,
          description:
            "NAVIGATION: find files by path/name or by keyword in content inside the vault. " +
            "Use when you do not know the exact path. " +
            "mode=glob: match paths/names (e.g. '*.md', 'RAG'). " +
            "mode=content: ripgrep-like search inside files (exact terms, headings, quotes). " +
            "Default returns paths or path:line snippets only — then use filesystem-read-text-file to deep-read. " +
            "Avoid includeFileContents unless you truly need one quick peek; prefer explicit read for rewriting. " +
            "Simple names without wildcards match *name* anywhere in the path.",
          examples: [
            {
              prompt: "Find markdown notes about RAG",
              call: JSON.stringify({
                pattern: "*RAG*",
                mode: "glob",
            ...
            {
              prompt: "Find where Embedding is mentioned",
              call: JSON.stringify({
                pattern: "Embedding",
                mode: "content",
                filePattern: "*.md",
          ...
          parameters: {
            ...
            properties: {
              ...
              filePattern: {
                type: "string",
                description:
                  "For content mode only: glob pattern to filter which files to search (e.g., '*.md').",
              },
              excludePatterns: {
                type: "array",
                items: { type: "string" },
                default: [],
                description:
                  "Patterns to exclude from search (e.g., 'node_modules', '*.log', '.git').",
              },
              caseSensitive: {
                type: "boolean",
                default: true,
              ...
              maxResults: {
                type: "number",
                default: 30,
                description: "Maximum number of results to return (keep modest for navigation).",
              },
              includeFileContents: {
                type: "boolean",
                default: false,
                description:
                  "Prefer false. If true, also load full text of a few matches (maxFilesToRead). " +
                  "For understanding or rewriting, call filesystem-read-text-file on chosen paths instead.",
              },
              maxFilesToRead: {
                type: "number",
                default: 2,
                description:
                  "When includeFileContents is true, max files to load (keep small).",
              },
            },
            required: ["pattern"],
