  ...
  plugin: function () {
    return {
      ...
      _store: async function (
        ...
        const cleanText = sanitizeAgentFinalText(response);
        let refinedSources = [];
        if (shouldAttachAgentSources(prompt)) {
          refinedSources = refineCitationsWithAnswer(
            cleanText,
            dedupeCitations(citations)
          );
        }
        aibitat._pendingCitations = refinedSources;
      ...
      _storeSpecial: async function (
        ...
        const rawText = options.hasOwnProperty("storedResponse")
          ? options.storedResponse(response)
          : response;
        const cleanText = sanitizeAgentFinalText(
          typeof rawText === "string" ? rawText : String(rawText ?? "")
        );
        let sources = [];
        if (shouldAttachAgentSources(prompt)) {
          sources = refineCitationsWithAnswer(
            cleanText,
            dedupeCitations([...existingSources, ...citations])
          );
        }
        aibitat._pendingCitations = sources;
        await WorkspaceChats.upsert(aibitat.trackedChatId, {
          workspaceId: Number(invocation.workspace_id),
          prompt,
          response: {
