...
} = require("./utils/toolCallSanitize.js");
const { formatToolActivityLabel } = require("./utils/toolActivityLabel.js");
...
class AIbitat {
  ...
  addCitation(citations) {
    if (!citations) return;
    const { dedupeCitations } = require("./utils/agentCitations.js");
    const incoming = Array.isArray(citations)
      ? citations.filter(Boolean)
      : typeof citations === "object"
        ? [citations]
        : [];
    // Merge + dedupe by path/id (keep longest body for later section refine)
    this._pendingCitations = dedupeCitations([
  ...
  flushCitations(messageUuid, answerText = "") {
    ...
    // 总结/修改/创建等操作：不向前端展示来源
    if (!shouldAttachAgentSources(userPrompt)) {
      this._pendingCitations = [];
      this.socket?.send?.("reportStreamEvent", {
        type: "citations",
        uuid: messageUuid,
    ...
    if (this._pendingCitations.length === 0) return;
    let citations = dedupeCitations(this._pendingCitations);
    if (answerText) {
      citations = refineCitationsWithAnswer(answerText, citations);
    }
    this._pendingCitations = citations;
    this.socket?.send?.("reportStreamEvent", {
      type: "citations",
      uuid: messageUuid,
      citations,
...
`
   ...
   * Transient network failures (ECONNRESET / timeouts) are retried a few times.
   *
  ...
  async #safeProviderCall(providerCall) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const isTransient =
          /ECONNRESET|ETIMEDOUT|ECONNREFUSED|socket hang up|Connection error|network|APIConnectionError/i.test(
            `${msg} ${code}`
        ...
        if (isTransient && attempt < maxAttempts) {
          ...
          console.warn(
            `[AIbitat] Provider connection error (attempt ${attempt}/${maxAttempts}), retry in ${delayMs}ms: ${msg}`,
            { hide_meta: true }
          );
        ...
        console.error(`[AIbitat] Provider error: ${msg}`, {
          hide_meta: true,
        });
        throw new APIError(`The agent model failed to respond: ${msg}`);
      }
    }
  ...
  async handleAsyncExecution(
    ...
    let completionStream = await this.#safeProviderCall(() =>
      this.providerInstance.stream(messages, functions, eventHandler)
    );
    completionStream = normalizeProviderCompletion(completionStream, functions);

    // Plain-text tool markup that we could not recover → one model-agnostic retry
    if (
      !completionStream.functionCall &&
      looksLikeLeakedToolMarkup(completionStream.textResponse) &&
      recoverAttempts < 1
    ) {
      this.handlerProps?.log?.(
        "[AIbitat] Recovering from plain-text tool call markup (retry once)."
      );
      this?.introspect?.(
        "检测到未通过原生工具协议的调用，正在按标准协议重试…"
      );
      return await this.handleAsyncExecution(
        [
          ...
          {
            role: "assistant",
            content: String(completionStream.textResponse || "").slice(0, 4000),
          },
          {
            role: "user",
            content: TOOL_PROTOCOL_RECOVERY_HINT,
          },
        ],
        functions,
        byAgent,
    ...
    if (completionStream.functionCall) {
      ...
      // Always emit a short Grok-style tool step for the chat activity strip
      this?.introspect?.(formatToolActivityLabel(name, args));
    ...
    {
      const finalText = sanitizeAgentFinalText(completionStream?.textResponse);
      this?.flushCitations?.(responseUuid, finalText);
  ...
  async handleExecution(
    ...
    let completion = await this.#safeProviderCall(() =>
      this.providerInstance.complete(messages, functions)
    );
    completion = normalizeProviderCompletion(completion, functions);

    if (
      !completion.functionCall &&
      looksLikeLeakedToolMarkup(completion.textResponse) &&
      recoverAttempts < 1
    ) {
      this.handlerProps?.log?.(
        "[AIbitat] Recovering from plain-text tool call markup (complete, retry once)."
      );
      this?.introspect?.(
        "检测到未通过原生工具协议的调用，正在按标准协议重试…"
      );
      return await this.handleExecution(
        [
          ...
          {
            role: "assistant",
            content: String(completion.textResponse || "").slice(0, 4000),
          },
          {
            role: "user",
            content: TOOL_PROTOCOL_RECOVERY_HINT,
    ...
    {
      const finalText = sanitizeAgentFinalText(completion?.textResponse);
      this?.flushCitations?.(msgUUID, finalText);
