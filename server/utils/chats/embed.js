...
function parseStructuredResponse(rawText) {
  ...
  if (openFence !== -1) text = text.slice(0, openFence).trim();

  return { text: text.trim(), supplement, relatedTopics, citations };
}

/**
 * Enhances a source object with citation metadata.
 * @param {Object} source
 * @param {number} idx
 * @returns {Object}
 */
...
async function streamChatWithForEmbed(
  ...
  const baseSystemPrompt = await chatPrompt(embed.workspace, username);

  const citationInstruction =
    contextTexts.length > 0
      ? `\n\nWhen using information from the provided Context, place inline citations immediately after the relevant sentence using square brackets with 1-based numbers: [1], [2], etc. Numbering maps as CONTEXT 0 → [1], CONTEXT 1 → [2], and so on. Only cite contexts you actually used. Do not invent citations. Do not append a JSON block or code-fenced metadata at the end.`
      : "";
  const messages = await LLMConnector.compressMessages(
    {
      systemPrompt: baseSystemPrompt + citationInstruction,
      userPrompt: message,
      contextTexts,
      chatHistory,
    },
  ...
  if (LLMConnector.streamingEnabled() !== true) {
    ...
    // Parse structured response for clean display text
    const nonStreamStructured = parseStructuredResponse(completeText);

    // Populate highlights on sources
    if (nonStreamStructured.citations && nonStreamStructured.citations.length > 0) {
      sources = sources.map((source) => {
        if (source.chunkId && nonStreamStructured.citations.includes(source.chunkId)) {
          return { ...source, highlights: [{ start: 0, end: (source.surroundingText || source.text || "").length }] };
        }
        return source;
      });
    }
    ...
    writeResponseChunk(response, {
      ...
      textResponse: nonStreamStructured.text,
      close: true,
      error: false,
    });
    ...
    await EmbedChats.new({
      ...
      prompt: message,
      response: {
        text: nonStreamStructured.text,
        type: chatMode,
        sources,
        metrics,
        ...(nonStreamStructured.supplement ? { supplement: nonStreamStructured.supplement } : {}),
        ...(nonStreamStructured.relatedTopics ? { relatedTopics: nonStreamStructured.relatedTopics } : {}),
      },
  ...
  // Populate highlights on sources based on citation matches
  if (structured.citations && structured.citations.length > 0) {
    sources = sources.map((source) => {
      if (source.chunkId && structured.citations.includes(source.chunkId)) {
        return { ...source, highlights: [{ start: 0, end: (source.surroundingText || source.text || "").length }] };
      }
      return source;
    });
