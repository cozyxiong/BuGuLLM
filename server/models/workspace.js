...
/** 历史 AnythingLLM 英文默认（用于识别并迁移到当前默认） */
...
const Workspace = {
  /** 问答(query) | 助手(assistant)。兼容旧值 chat→query、automatic→assistant */
  VALID_CHAT_MODES: ["query", "assistant"],
  /** 始终读最新代码默认，避免模块加载时缓存旧字符串 */
  get defaultPrompt() {
  ...
  validations: {
    name: (value) => {
      ...
      // Display name length matches frontend limit.
    ...
    chatMode: (value) => {
      // 兼容旧模式名
      if (value === "chat") return "query";
      if (value === "automatic") return "assistant";
  ...
  new: async function (name = null, creatorId = null, additionalFields = {}) {
    ...
    // 系统级 default_system_prompt（管理后台）> 代码 saneDefault
    // 若库里仍是历史旧默认文案，忽略并使用当前代码默认
    const defaultSystemPrompt = await SystemSettings.get({
      label: "default_system_prompt",
    });
    const storedDefault = defaultSystemPrompt?.value
      ? String(defaultSystemPrompt.value).trim()
      : "";
    const useStoredDefault =
      !!storedDefault && !LEGACY_DEFAULT_PROMPTS.includes(storedDefault);
    additionalFields.openAiPrompt = useStoredDefault
      ? storedDefault
      : SystemSettings.saneDefaultSystemPrompt;

    try {
      // chatMode 放在最后：默认 query，仅当创建时显式传入合法值才覆盖
      const fields = this.validateFields(additionalFields);
      // openAiPrompt 校验若异常丢字段，强制写回默认
      if (!fields.openAiPrompt) {
        fields.openAiPrompt = SystemSettings.saneDefaultSystemPrompt;
      }
      const workspace = await prisma.workspaces.create({
        data: {
          ...
          slug,
          chatMode:
            additionalFields?.chatMode &&
            this.VALID_CHAT_MODES.includes(additionalFields.chatMode)
              ? additionalFields.chatMode
              : "query",
        },
      });
  ...
  update: async function (id = null, updates = {}) {
    ...
    const unsettingProvider =
      Object.prototype.hasOwnProperty.call(updates, "chatProvider") &&
      ["default", "none", null, "", undefined].includes(updates.chatProvider);

    // 跟随系统 / 清空提供方时，一并清掉模型和绑定的路由
    if (unsettingProvider || validatedUpdates?.chatProvider === "default") {
    ...
    // 允许只解绑路由，不改提供方
    if (
      Object.prototype.hasOwnProperty.call(updates, "router_id") &&
      [null, undefined, "", "none"].includes(updates.router_id)
    ) {
      validatedUpdates.router_id = null;
    }
  ...
  /**
   * @agent 入口已废弃：问答模式不进 Agent，助手模式每条消息自动进 Agent。
   * 始终返回 false，前端隐藏 @agent 按钮。
   * @param {Workspace} _workspace
   * @returns {Promise<boolean>}
   */
  isAgentCommandAvailable: async function (_workspace) {
    return false;
  },
};
